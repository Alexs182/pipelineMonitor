...
// ── apiFetch — defined first so everything below can use it ──────────────────

function apiFetch(path) {
  const dbName = $('db-select')?.value || '';
...
async function loadDbList() {
  ...
  try {
    ...
    dbSelect.innerHTML = conns.length
      ? conns.map(c => `<option value="${c.name}">${c.label}</option>`).join('')
      : '<option value="">No DBs configured</option>';

    const saved = sessionStorage.getItem('selectedDb');
    if (saved && conns.find(c => c.name === saved)) dbSelect.value = saved;

  } catch {
    dbSelect.innerHTML = '<option value="">Error loading</option>';
  }
}

$('db-select')?.addEventListener('change', () => {
  sessionStorage.setItem('selectedDb', $('db-select').value);
  fetchAll();
});
...
function renderTables(rows) {
  ...
  if (!rows.length) { list.innerHTML = '<tr><td colspan="4" class="empty">No tables found</td></tr>'; return; }
...
function filterRowCount() {
  ...
  const rows = $('table-list').getElementsByTagName('tr');
  ...
  container.innerHTML = data.map(q => `
    <div class="query-row">
      ...
      <div class="query-stats">
        <span class="qstat"><span class="qstat-label">avg</span>&nbsp;<span class="qstat-value ${q.avg_ms > 500 ? 'red' : q.avg_ms > 100 ? 'amber' : ''}">${q.avg_ms}ms</span></span>
...
function renderPipeline(data) {
  ...
  $('failed-jobs').innerHTML = (data.recentFailed || []).map(j => `
...
async function fetchAll() {
  ...
  try {
    ...
    renderStatus(
      statusRes.status === 'fulfilled' && statusRes.value.success
    ...
    if (tableRes.status === 'fulfilled' && tableRes.value.success)
      ...
      $('table-list').innerHTML = '<tr><td colspan="4" class="empty" style="color:var(--red)">Failed to load tables</td></tr>';

    renderSlowQueries(
      sqRes.status === 'fulfilled' && sqRes.value.success ? sqRes.value.data
        : sqRes.status === 'fulfilled' ? { error: sqRes.value.error } : null
    );

    renderPipeline(
      pipelineRes.status === 'fulfilled' && pipelineRes.value.success ? pipelineRes.value.data
        : pipelineRes.status === 'fulfilled' ? { error: pipelineRes.value.error } : null
    );

    $('fetched-at').style.display = 'block';
    $('fetched-at').textContent = 'Last refreshed ' + new Date().toLocaleTimeString();
...
loadDbList().then(() => { if (apiKey) fetchAll(); });